/*
 * GPIO control - green LED
 * MX93_PAD_GPIO_IO05__GPIO2_IO04
 */
#include <stdio.h>
#include <errno.h>
#include <string.h>
...
struct gpiod_line_request *led_line_request;

const char *gpio_chip_path = "/dev/gpiochip0";
const uint16_t gpio_pin_offset = 4;
...
int gpio_init(const char* appname)
{
    ...
    led_line_request = gpiod_chip_request_lines(chip_v2, req_cfg, line_cfg);
...
int led(uint8_t state)
{
    ...
    if(gpiod_line_request_set_values(led_line_request, values) < 0)
    {
