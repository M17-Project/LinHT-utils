/*
 * GPIO control - TX/RX RF switch
 * MX93_PAD_GPIO_IO13__GPIO2_IO13
 */
#include <stdio.h>
#include <errno.h>
#include <string.h>
...
struct gpiod_line_request *txrx_line_request;

const char *gpio_chip_path = "/dev/gpiochip0";
const uint16_t gpio_pin_offset = 13;
...
int gpio_init(const char* appname)
{
    ...
    txrx_line_request = gpiod_chip_request_lines(chip_v2, req_cfg, line_cfg);
...
void gpio_cleanup(void)
{
    if(txrx_line_request)
	{
        gpiod_line_request_release(txrx_line_request);
    }
    if(chip_v2)
	{
        gpiod_chip_close(chip_v2);
